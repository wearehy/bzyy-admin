<template>
  <div class="app-container">
    ...
    <!--表单渲染-->
    <el-dialog append-to-body :close-on-click-modal="false" :before-close="handelCancel" :visible.sync="dialog" :title="dialogTitle" width="580px">
      <el-form ref="form" :inline="true" :model="form" :rules="rules" size="small" label-width="78px">
        ...
        <el-form-item label="上级类目" prop="pid" required>
          <treeselect v-model="form.pid" :options="cates" style="width: 390px;" placeholder="选择上级类目" />
        </el-form-item>
        <el-form-item label="分类图片" prop="image">
          <el-upload 
                ...
                class="upload-demo"
              >
                <img :src="form.image ? form.image : defaultPic" title="点击上传图片" class="avatar" style="width:140px;height:140px">
              </el-upload>
        </el-form-item>
    ...
    <!--表格渲染-->
    <el-table ref="table" v-permission="['/cate']" v-loading="loading" :data="tableData" :tree-props="{children: 'children', hasChildren: 'hasChildren'}" row-key="id">
      ...
      <el-table-column label="操作" width="130px" align="center" fixed="right">
        <template slot-scope="scope">
          <el-button :disabled="$permissionButton('/cate/edit')" size="mini" type="primary" icon="el-icon-edit" @click="handelEdit(scope.row)" />
          <el-button :disabled="$permissionButton('/cate/del')" size="mini" type="danger" icon="el-icon-delete" @click="handelDel(scope.row)" />
        </template>
      </el-table-column>
    </el-table>
  </div>
...
const defaultForm = {id: '',pid: 0,title: '',image: '',display:true,listorder: 999}
export default {
  name: 'Category',
  inject:['reload'],
