...
export default {
  data() {
    return {
      ...
      options: {
        autoCropWidth: this.share.autoCropWidth || 200,   //默认生成截图框宽度
        autoCropHeight: this.share.autoCropHeight || 200, //默认生成截图框高度
        info: this.share.info || true,  //显示裁剪框的大小信息
        outputSize: this.share.outputSize || this.cutOutputSize,  //裁剪生成图片的质量 (0.1 - 1)
        outputType: this.share.outputType || "jpeg", //裁剪生成图片的格式
        centerBox: this.share.centerBox || false, //截图框是否被限制在图片里面
        canScale: this.share.canScale || true, //图片是否允许滚轮缩放
        autoCrop: this.share.autoCrop || true, // 是否默认生成截图框, 只有自动截图开启 宽度高度才生效
        fixedBox: this.share.fixedBox || false, //  固定截图框大小 不允许改变
        fixed: this.share.fixed || true, // 开启宽度和高度比例
        fixedNumber: this.share.fixedNumber || [1, 1],  //截图框的宽高比例
        enlarge: this.share.enlarge || 1,  //图片根据截图框输出比例倍数
        original: this.share.original || false, // 上传图片按照原始比例渲染
        full: this.share.full || false, // 是否输出原图比例的截图

        canMove: this.share.canMove || true,//上传图片是否可以移动
        canMoveBox: this.share.canMoveBox || true,//截图框能否拖动
        high: this.share.high || true,  //是否按照设备的dpr 输出等比例图片
        infoTrue: this.share.infoTrue || false, //true 为展示真实输出图片宽高 false 展示看到的截图框宽高
