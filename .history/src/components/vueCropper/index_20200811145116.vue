  ...
  data() {
    return {
      ...
      options: {
        ...
        outputSize: this.share.outputSize || this.cutOutputSize, //裁剪生成图片的质量 (0.1 - 1)
        outputType: this.share.outputType || "jpeg", //裁剪生成图片的格式
        centerBox: (typeof this.share.centerBox === 'boolean') ?this.share.centerBox: false, //截图框是否被限制在图片里面
        canScale: (typeof this.share.canScale === 'boolean') ?this.share.canScale: true, //图片是否允许滚轮缩放
        autoCrop: (typeof this.share.autoCrop === 'boolean') ?this.share.autoCrop: true, // 是否默认生成截图框, 只有自动截图开启 宽度高度才生效
        fixedBox: (typeof this.share.fixedBox === 'boolean') ?this.share.fixedBox: false, //  固定截图框大小 不允许改变
        ...
        original: (typeof this.share.original === 'boolean') ?this.share.original: false,// 上传图片按照原始比例渲染
        full: (typeof this.share.full === 'boolean') ?this.share.full: false,// 是否输出原图比例的截图

        canMove: (typeof this.share.canMove === 'boolean') ?this.share.canMove: true,//上传图片是否可以移动
        canMoveBox: (typeof this.share.canMoveBox === 'boolean') ?this.share.canMoveBox: true,//截图框能否拖动
        high: (typeof this.share.high === 'boolean') ?this.share.high: true,//是否按照设备的dpr 输出等比例图片
        infoTrue: (typeof this.share.infoTrue === 'boolean') ?this.share.infoTrue: false,//this.share.infoTrue || false,//true 为展示真实输出图片宽高 false 展示看到的截图框宽高
